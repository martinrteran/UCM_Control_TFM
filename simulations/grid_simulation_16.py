...
rl_config = PPOConfig(obs_dim=obv_dim, action_dim=action_dim, algorithm=RLAlgorithm.PPO,
                     device=device,name="PPO Agent Obstacles 5k", batch_size=64, lr=1e-3, gamma=0.99,
                     eps_start=1.0, eps_end=0.05, eps_decay=1_000_000, buffer_size=100_000,
                     max_grad_norm=5.0)

policy_net = SimpleQNetwork(obs_dim=obv_dim, action_dim=action_dim, num_hidden_layers=3, hidden_dim=128).to(device)
target_net = SimpleQNetwork(obs_dim=obv_dim, action_dim=1, num_hidden_layers=3, hidden_dim=128).to(device)
agent = PPOAgent(rl_config, policy_network = policy_net, value_network = target_net)
agent.load(rf"./checkpoints/Test/16/PPO Agent Obstacles 5k/final.pth")
...
singleTrainer = training_manager.PPOTrainer("./runs/Test/16","./checkpoints/Test/16",t_config)
