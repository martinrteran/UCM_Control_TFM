...
rl_config = PPOConfig(obs_dim=obv_dim, action_dim=action_dim, algorithm=RLAlgorithm.PPO,
                     device=device,name="PPO Agent 5k", batch_size=64, lr=1e-3, gamma=0.99,
...
agent.load(rf"./checkpoints/Test/17/PPO Agent 5k/final.pth")
...
singleTrainer = training_manager.PPOTrainer("","",t_config)

singleTrainer.simulate(agent,env,10,True)
