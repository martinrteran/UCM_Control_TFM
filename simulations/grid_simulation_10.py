...
env = grid_env.Grid_Robot_Sections_Env(map=mapa_simple, robot=robot_simple, cell_size=10, max_iteration_steps=100, render_mode="human")
...
rl_config = RLConfig(obs_dim=obv_dim, action_dim=action_dim, algorithm=RLAlgorithm.GRID_AGENT,
                     device=device,name="Grid_Agent", double_dqn = False, batch_size=64, lr=1e-3, gamma=0.99,
...
agent.load(rf"checkpoints\Test\10\Grid_Agent\20260707-204504\ep_49500.pth")

t_config = training_manager.TrainerConfig(
    eps_step=10,
    num_episodes=50_000,
)
singleTrainer = training_manager.SingleTrainer("./runs/Test/10","./checkpoints/Test/10",t_config)
