...
rl_config = PPOConfig(obs_dim=obv_dim, action_dim=action_dim, algorithm=RLAlgorithm.PPO,
                     device=device,name="PPO Agent timer", batch_size=64, lr=1e-3, gamma=0.99,
...
agent.load(rf"checkpoints\Test\20\PPO Agent timer\final.pth")
...
singleTrainer = training_manager.PPOTrainer("./runs/Test/20","./checkpoints/Test/20",t_config)

singleTrainer.simulate(agent=agent, env=env, num_episodes=50, render=True)


mapa_simple = maps.GridMap.load("./maps/map_1_obstacle.json")
robot_simple = robot.GridRobotNotTurning()
n = 2 # 3
n_sections = 4 * n
lidar_config = {"num_rays": n_sections*15, "max_range": 10, "fov": 2*np.pi, "noise_std": 1e-3, "with_cache": True}
...
mapa_simple = maps.GridMap.load("./maps/map_obstacle_free.json")
robot_simple = robot.GridRobotNotTurning()
n = 2 # 3
n_sections = 4 * n
lidar_config = {"num_rays": n_sections*15, "max_range": 10, "fov": 2*np.pi, "noise_std": 1e-3, "with_cache": True}
