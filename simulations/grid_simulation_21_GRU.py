from tfm_control_ucm.core.agents.agent import  PPOAgent,PPOGRUAgent
...
rl_config = PPOConfig(obs_dim=obv_dim, action_dim=action_dim, algorithm=RLAlgorithm.PPO,
                     device=device,name="PPO GRU Agent", batch_size=64, lr=1e-3, gamma=0.99,
...
agent = PPOGRUAgent(rl_config, policy_network = policy_net, value_network = target_net)
agent.load(rf"checkpoints\Test\21\PPO GRU Agent\final.pth")

t_config = training_manager.TrainerConfig(
    num_episodes=40_000,
    render=True
)
singleTrainer = training_manager.PPOGRUTrainer("./runs/Test/21","./checkpoints/Test/21",t_config)
