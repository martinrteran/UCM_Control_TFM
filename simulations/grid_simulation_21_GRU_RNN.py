from tfm_control_ucm.core.agents.agent import  PPOAgent,PPOGRUAgent, RecurrentPPOAgent
from tfm_control_ucm.core.agents.networks import RecurrentGRUPolicy,RecurrentGRUValue
...
rl_config = PPOConfig(obs_dim=obv_dim, action_dim=action_dim, algorithm=RLAlgorithm.PPO,
                     device=device,name="PPO GRU RNN Agent 20k", batch_size=64, lr=1e-3, gamma=0.99,
                     eps_start=1.0, eps_end=0.05, eps_decay=1_000_000, buffer_size=100_000,
                     max_grad_norm=5.0)

policy_net = RecurrentGRUPolicy(obs_dim=obv_dim, action_dim=action_dim, hidden_dim=64).to(device)
target_net = RecurrentGRUValue(obs_dim=obv_dim, hidden_dim=64).to(device)
agent = RecurrentPPOAgent(rl_config, policy_net, target_net)
agent.load(rf"./checkpoints/Test/21/PPO GRU RNN Agent 20k/final.pth")

t_config = training_manager.TrainerConfig(
    num_episodes=20_000,
    render=True
)
singleTrainer = training_manager.RecurrentPPOTrainer("./runs/Test/21","./checkpoints/Test/21",t_config)
