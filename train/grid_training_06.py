...
lidar_config = {"num_rays": 30, "max_range": 10, "fov": 2*np.pi, "noise_std": 1e-3, "with_cache": False}
env = grid_env.Grid_Robot_Sections_Env(map=mapa_simple, robot=robot_simple, cell_size=10, lidar_config=lidar_config)
...
rl_config = RLConfig(obs_dim=obv_dim, action_dim=action_dim, algorithm=RLAlgorithm.GRID_AGENT,
                     device=device,name="Grid_Agent", double_dqn = False, batch_size=int(64*1.5), lr=1e-2, gamma=0.95,
                     eps_start=1.0, eps_end=-1e-6, eps_decay=100_000, target_update=1000, buffer_size=1_000_000)
...
singleTrainer = training_manager.SingleTrainer("./runs/Test/06/100k","./checkpoints/Test/06/100k",t_config)
