...
mapa_complejo = maps.GridMap.load("./src/tfm_control_ucm/maps/map_simple.json")
robot_simple = robot.GridRobot()
lidar_config = {"num_rays": 30, "max_range": 10, "fov": 2*np.pi, "noise_std": 1e-3, "with_cache": True, "device": device}
env = grid_env.Grid_Robot_Sections_Env(map=mapa_simple, robot=robot_simple, cell_size=10)
env_complejo = grid_env.Grid_Robot_Sections_Env(map=mapa_complejo, robot=robot_simple, cell_size=10)
...
rl_config = RLConfig(obs_dim=obv_dim, action_dim=action_dim, algorithm=RLAlgorithm.GRID_AGENT,
                     device=device,name="GridAgent", double_dqn = False, batch_size=64, lr=1e-3, gamma=0.99,
                     eps_start=1.0, eps_end=0, eps_decay=200_000, target_update=500, buffer_size=100_000)
...
multiEnvTrainer =training_manager.MultiEnvironmentTrainer("./runs/Test/09","./checkpoints/Test/09",t_config).add_agents(agent).add_environments(env,env_complejo)
multiEnvTrainer.train()