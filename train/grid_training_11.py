...
env = grid_env.Grid_Robot_Sections_Env(map=mapa_simple, robot=robot_simple, cell_size=10, max_iteration_steps=200, num_sections=8, lidar_config=lidar_config, render_mode="human")
...
policy_net = SimpleQNetwork(obs_dim=obv_dim, action_dim=action_dim, num_hidden_layers=2, hidden_dim=4).to(device)
target_net = SimpleQNetwork(obs_dim=obv_dim, action_dim=action_dim, num_hidden_layers=2, hidden_dim=4).to(device)
...
singleTrainer = training_manager.SingleTrainer("./runs/Test/11","./checkpoints/Test/11",t_config)
